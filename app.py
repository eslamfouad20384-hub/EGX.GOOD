...
def run_real_backtest(df, max_bars=300):
    """
    Event-driven backtest with no look-ahead:
    signal is evaluated on bar t, entry is executed at bar t+1 open,
    and exits are checked only on subsequent OHLC bars.
    Targets use structural levels available at the signal bar only.
    """
    if df is None or df.empty or len(df) < BACKTEST_WARMUP + 30:
        return {"trades": 0, "win_rate": 0.0, "profit_pct": 0.0, "max_drawdown_pct": 0.0, "profit_factor": 0.0}

    data = df.tail(max_bars + BACKTEST_WARMUP).copy()
    trades=[]
    equity=1.0
    peak=1.0
    max_dd=0.0
    ...
    for i in range(BACKTEST_WARMUP, len(data)-2):
        row=data.iloc[i]
        if not all(np.isfinite(float(row.get(c, np.nan))) for c in ["Close","ema20","ema50","rsi","macd","macd_signal","atr"]):
            continue
        close=float(row["Close"]); atr_val=float(row["atr"]);
        if atr_val <= 0: continue

        prev_res=float(row.get("previous_resistance", np.nan))
        breakout=(np.isfinite(prev_res) and close > prev_res and float(row.get("volume_ratio",0)) >= 1.5 and float(row.get("body_pct",0)) >= 0.45 and close > row["ema20"] > row["ema50"])
        pullback=bool(row.get("pullback",False)) and row["ema20"] > row["ema50"]
        immediate=(close > row["ema20"] > row["ema50"] and row["rsi"] >= 50 and row["macd"] > row["macd_signal"] and float(row.get("volume_ratio",0)) >= 1.0)
        if not (breakout or pullback or immediate):
            continue

        entry=float(data.iloc[i+1]["Open"])
        if not np.isfinite(entry) or entry <= 0: continue
        stop_level=float(row.get("confirmed_swing_low", np.nan))
        if np.isfinite(stop_level) and stop_level < entry:
            stop=max(stop_level-atr_val*0.2, entry-atr_val*1.5)
        else:
        ...
        for j in range(i+2, len(data)):
            bar=data.iloc[j]
            # Conservative ordering when both stop and target are touched in same bar: stop first.
            if float(bar["Low"]) <= stop:
                outcome="loss"; exit_price=stop; exit_i=j; break
            if float(bar["High"]) >= target:
                outcome="win"; exit_price=target; exit_i=j; break
        if outcome is None:
            exit_price=float(data.iloc[-1]["Close"]); exit_i=len(data)-1; outcome="open"

        ret=(exit_price-entry)/entry
        if outcome == "loss": ret=min(ret,0)
        equity *= (1+ret)
        peak=max(peak,equity); max_dd=max(max_dd,(peak-equity)/peak)
        trades.append({"return":ret,"outcome":outcome,"entry":entry,"exit":exit_price,"bars":exit_i-(i+1)})
